...
double weight_update(double sig_x, double sig_y, double x_obs, double y_obs, double mu_x, double mu_y) {
  ...
  for (unsigned int i = 0; i < particles.size(); ++i)
    ...
    double prob = 1.0;
        for (unsigned int j = 0; j < observations.size(); j++)
        {
            ...
            // x-map is the x-coordinate of the landmark for the particle pi. observations[j] is the actual position of a landmark.
          // Read map data from helper_functions.h use same nomenclature
            ...
            for (unsigned int k = 0; k < landmark_list.size(); k++)
            {
                ...
                if ((distance <= sensor_range) && (distance <= max_val))
                {
                  ...
                  land_y = local_land_y;
                  max_val = distance;
                  prob = weight_update(std_landmark[0], std_landmark[1],x_map, y_map, land_x, land_y);
